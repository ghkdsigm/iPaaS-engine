...
export class ApprovalsService {
  ...
  list() {
    return this.prisma.approval.findMany({ orderBy: { createdAt: "desc" }, include: { plan: { include: { command: true } } } }).then(a => ({ approvals: a }));
  }

  async approve(id: string) {
    const approval = await this.prisma.approval.findUnique({ where: { id }, include: { plan: { include: { command: true } } } });
    if (!approval) return { ok: false };
    if (approval.status !== "PENDING") return { ok: true, status: approval.status };

    await this.prisma.approval.update({ where: { id }, data: { status: "APPROVED", resolvedAt: new Date() } });
  ...
  async reject(id: string) {
    await this.prisma.approval.update({ where: { id }, data: { status: "REJECTED", resolvedAt: new Date() } });
    return { ok: true };
  }
}
